...
| `mobius` 🧪 | Experimental — learning rate **is** the Klein-Maimon Möbius measurement coordinate of the soft state: `lr_t = base_lr · \|B\|`, `B = b·sign(a)/(\|a\|+\|b\|)`. Touches **only** the learning rate (no gradient warp); bounded by construction (`lr ≤ base_lr`); anneals the rate down as improvement slows. Tends to match `adaptive` on rugged/long-horizon problems. Set `base_lr` near the top of a stable range. | 0.01 | 0.02 |
...
> 🧪 **`mobius` is experimental.** It is the minimal sibling of `pure`: the learning rate is read directly off the Möbius **measurement** coordinate `B` of the soft state (`lr_t = base_lr · |B|`), and the gradient is passed through **unwarped**. Because `|B| ≤ 1`, the rate is bounded by construction (`lr ≤ base_lr`) and anneals downward as realized improvement slows relative to curvature. In offline tests this matches the scalar `adaptive` layer and, on rugged landscapes with a long horizon, can beat a best-tuned fixed learning rate without any tuning; on smooth landscapes the bound can cost a little versus a high constant rate, so pick `base_lr` near the top of a stable range. The signal mapping, evolution law, and base optimizer are identical to the other methods — only the final state → lr step differs (and there is no warp).
