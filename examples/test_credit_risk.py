...
- Baseline: Pure SPSA optimizer (what customer has BEFORE adding Mobiu-Q)
...
LR = 0.2
BOUNDS = (-5.0, 5.0)
FUNCTION = "credit_risk"  # portfolio, credit_risk, option_pricing
METHOD = "standard"  # deep works better for rugged landscapes
MAXIMIZE = False  # For risk minimization
...
class LandscapeFunctions:
    ...
    @staticmethod
    def credit_risk(x: np.ndarray) -> float:
        """Credit risk objective: minimize simulated risk function"""
        n = len(x)
        probabilities = 1 / (1 + np.exp(-x))  # logistic for default prob
        correlations = np.outer(probabilities, probabilities) * 0.1
        risk = np.sum(probabilities) + x.T @ correlations @ x
        return risk  # to minimize
...
def main():
    ...
    # Select function
    functions = {
        'credit_risk': LandscapeFunctions.credit_risk,
    }
