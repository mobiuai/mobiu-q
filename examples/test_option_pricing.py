...
LR = 0.2
BOUNDS = (-5.0, 5.0)
FUNCTION = "option_pricing"  # portfolio, credit_risk, option_pricing
METHOD = "standard"  # deep works better for rugged landscapes
MAXIMIZE = True  # For pricing maximization (e.g., value)
...
class LandscapeFunctions:
    ...
    @staticmethod
    def option_pricing(x: np.ndarray) -> float:
        """Option pricing objective: maximize simulated option value (simplified Black-Scholes like)"""
        n = len(x)
        S = np.sum(x) + 100  # stock price proxy
        K = 100  # strike
        r = 0.05  # risk-free rate
        sigma = 0.2 + 0.01 * np.std(x)  # volatility
        T = 1.0  # time
        d1 = (np.log(S/K) + (r + sigma**2/2)*T) / (sigma * np.sqrt(T))
        d2 = d1 - sigma * np.sqrt(T)
        from scipy.stats import norm
        call = S * norm.cdf(d1) - K * np.exp(-r*T) * norm.cdf(d2)
        return call  # to maximize
...
def main():
    ...
    # Select function
    functions = {
        'option_pricing': LandscapeFunctions.option_pricing,
    }
