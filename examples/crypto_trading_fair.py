...
"""
================================================================================
📦 PPO CRYPTO TRADING (BTC-like) - Improved Full PPO + Mobiu-Q [FAIR BENCHMARK]
================================================================================
"""
...
NUM_EPISODES = 600
NUM_SEEDS = 20                    # Increased for better statistics
BASE_LR = 0.0003
METHOD = "adaptive"
EPISODE_LENGTH = 600
...
class CryptoTradingEnv:
    ...
    def _generate_prices(self):
        """Crypto-like price generation (BTC style)"""
        prices = [100.0]
        for i in range(self.episode_length):
            regime = (i // 90) % 4
            if regime == 0:      # Strong bull
                ret = 0.0032 + 0.045 * self.rng.randn()
            elif regime == 1:    # High volatility
                ret = 0.0001 + 0.072 * self.rng.randn()
            elif regime == 2:    # Bear market
                ret = -0.0025 + 0.058 * self.rng.randn()
            else:                # Consolidation
                ret = 0.0011 + 0.023 * self.rng.randn()
            
            # Large jumps (typical in crypto)
            if self.rng.rand() < 0.011:
                ret += self.rng.choice([-0.135, 0.135])
...
def main():
    print("=" * 95)
    print("📦 PPO CRYPTO TRADING (BTC-like) - Improved PPO + Mobiu-Q [FAIR BENCHMARK]")
    print("=" * 95)
    ...
    for seed in range(NUM_SEEDS):
        ...
        # Save RNG state for perfect fairness
        ...
        # ------------------- Adam + Mobiu-Q -------------------
        # Restore RNG state so both see identical episodes
