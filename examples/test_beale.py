...
N_SEEDS = 10
N_STEPS = 100
N_PARAMS = 2  # Beale is typically 2D
SHOTS = 1024
NOISE_SCALE = 0.1
C_SHIFT = 0.1
LR = 0.2
BOUNDS = (-4.5, 4.5)  # Typical bounds for Beale
FUNCTION = "beale"  # rosenbrock, sphere, ackley, beale
METHOD = "deep"  # deep works better for rugged landscapes
...
class LandscapeFunctions:
    ...
    def beale(x: np.ndarray) -> float:
        """Beale: multimodal test function for optimization (typically 2D)"""
        if len(x) < 2:
            raise ValueError("Beale requires at least 2 dimensions")
        x1, x2 = x[0], x[1]
...
def main():
    ...
    functions = {
        ...
        'beale': LandscapeFunctions.beale,
    }
